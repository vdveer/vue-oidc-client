...
module.exports = __WEBPACK_EXTERNAL_MODULE_c097__;
...
function matchesPath(testUrl) {
  return window.location.pathname.toLocaleLowerCase() === getUrlPath(testUrl).toLocaleLowerCase();
}
...
function createOidcAuth(_authName, defaultSignInType, _appUrl, oidcConfig) {
  ...
  external_oidc_["Log"].level = logLevel;
  external_oidc_["Log"].logger = logger; // merge passed oidcConfig with defaults
  ...
  mgr.events.addSilentRenewError(function (e) {
    ...
    if (auth.isAuthenticated) {
      setTimeout(function () {
        external_oidc_["Log"].debug("".concat(_authName, " auth silent renew retry"));
        mgr.signinSilent();
      }, 5000);
    } else {
      signInIfNecessary();
    ...
    methods: {
      ...
      useRouter: function useRouter(router) {
        ...
        if (config.redirect_uri) {
          ...
          router.addRoutes([{
            path: vroutePath,
            name: "signinwin-".concat(_authName),
            component: {
